...
\lstdefinestyle{ABML} {
    ...
    morekeywords={
        ...
        push-aclosure, pop-aclosure, peek-aclosure
    },
    keywordstyle=\color{abmlKeyword}\bfseries,
    classoffset=1,
    morekeywords={
        :av, :apath, :do, :at, :atv, :b, :this, :ap, :p, :atp, :attributes, 
        :tags, :opt, :flatten, :case, :agent, :context, :union, :enum, :v, :t, 
        :p, :exit, :amap, :value, :nmatch, :attribute, :type, :stage, :instance, 
    ...
    morekeywords={
        ...
        logxor, lognot, logeqv, logorc, coerce
    },
...
Операционная семантика определяет, как вычисляются выражения языка Go в процессе 
выполнения программы. В данном разделе на языке ABML формализуются правила 
вычисления для всех конструкций языка: литералов, выражений, деклараций, операторов.

Вычисление происходит в контексте окружения — агента, который хранит информацию 
о переменных, константах, функциях и типах. Каждый шаг вычисления представлен 
атрибутным замыканием (\ABMLKeyword{aclosure}), которое задает атрибут 
\ABMLString{opsem::rvalue} для выражений, возвращающих значения, 
\ABMLString{opsem::lvalue} для получения ячеек памяти, и \ABMLString{opsem} 
...
\subsection{Литерал массива}
Вычисление литерала массива происходит в несколько этапов. Сначала вычисляется 
значение по умолчанию для элементов массива на основе типа элементов.
...
Затем создается массив из ячеек памяти, заполненных значениями по умолчанию.
...
На этапе вычисления явных элементов происходит замена значений 
...
\subsection{Литерал среза}
Вычисление литерала среза аналогично вычислению массива. Сначала вычисляется 
...
По максимальному индексу создается массив нужного размера.
...
На последнем этапе создается значение среза, ссылающееся на созданный массив.
...
Финальное значение структуры создается после обработки всех полей.
...
\subsection{Литерал карты}
Вычисление литерала карты происходит путем последовательного вычисления пар 
ключ-значение.
...
После вычисления всех пар создается итоговая карта.
...
\subsection{Литерал функции}
Функциональные литералы создают значения функций, сохраняя текущее состояние 
замыкания — ячейки памяти всех переменных, видимых в момент создания.
...
Для ссылки на переменную определены два замыкания: для получения левого значения 
(lvalue) — ячейки памяти, и для получения правого значения (rvalue) — содержимого 
ячейки.
